        ...
        <form action="{{ route('my-jobs.store') }}" method="POST">
            ...
            <div class="mb-4 grid grid-cols-2 gap-4">
                <div>
                    ...
                    id="title" type="text" placeholder="Enter your job title" name="title">
                    @error('title')
                ...
                <div>
                    ...
                    id="location" type="text" placeholder="Enter your location" name="location">
                    @error('location')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="col-span-2">
                ...
                id="salary" type="number" placeholder="Enter your salary" name="salary">
                @error('salary')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="col-span-2">
                ...
                <textarea id="description" name="description" rows="4" class="shadow appearance-none border rounded w-full py-2 px-3 mt-2 
                 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('description') border-red-500 @enderror" placeholder="Enter your description"></textarea>
                @error('description')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4 grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    @foreach($experienceOptions as $label => $value)
                        <div class="flex items-center mb-2">
                            <input id="experience_{{ $value }}" type="radio" name="experience" value="{{ $value }}" class="form-radio h-4 w-4 text-blue-600 transition duration-150 ease-in-out" {{ old('experience') === $value ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    @foreach($languages as $languages)
                        <div class="flex items-center mb-2">
                            <input id="language_{{ $languages }}" type="radio" name="languages" value="{{ $languages }}" class="form-radio h-4 w-4 text-blue-600 transition duration-150 ease-in-out" {{ old('languages') === $languages ? 'checked' : '' }}>
                            <label for="language_{{ $languages }}" class="ml-2 text-gray-700">{{ $languages }}</label>
                        </div>
                    @endforeach
                    @error('languages')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
            ...
            <button type="submit" class="bg-sky-500 w-full text-white font-small py-2">Create Job</button>
        </form>
    </x-card>
</x-layout>